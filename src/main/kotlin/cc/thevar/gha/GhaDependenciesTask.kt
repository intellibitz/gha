...
abstract class GhaDependenciesTask : GhaTask() {
    ...
    fun execute() {
        ...
        logger.lifecycle("🔒 [GHA Trusted Vendors & Official Stable Dependencies]")
        logger.lifecycle("┌" + "─".repeat(24) + "┬" + "─".repeat(12) + "┬" + "─".repeat(30) + "┬" + "─".repeat(14) + "┬" + "─".repeat(28) + "┬" + "─".repeat(22) + "┐")
        logger.lifecycle("│ %-22s │ %-10s │ %-28s │ %-12s │ %-26s │ %-20s │".format("Tool / Dependency", "Category", "Trusted Vendor", "Configured", "Runtime / Active", "Status"))
        logger.lifecycle("├" + "─".repeat(24) + "┼" + "─".repeat(12) + "┼" + "─".repeat(30) + "┼" + "─".repeat(14) + "┼" + "─".repeat(28) + "┼" + "─".repeat(22) + "┤")

        dependencies.forEach { dep ->
            val statusLabel = if (dep.isOfficialStable) "✅ Official Stable" else "⚠️ Unstable / Preview"
            logger.lifecycle("│ %-22s │ %-10s │ %-28s │ %-12s │ %-26s │ %-20s │".format(dep.name, dep.category, dep.vendor, dep.configuredVersion, dep.status, statusLabel))
        }
