...
@DisableCachingByDefault(because = "Reopens a closed Pull Request on GitHub")
abstract class GhaPrReopenTask : GhaTask() {
    ...
    fun execute() {
        ...
        logger.lifecycle("🔓 [GHA PR Reopen] Reopening Pull Request ${num?.let { "#$it" } ?: "current branch"}...")

        val cmd = mutableListOf("gh", "pr", "reopen")
        ...
        if (result.isSuccess) {
            logger.lifecycle("✅ ${result.stdout.ifEmpty { "Pull Request reopened successfully." }}")
        } else {
