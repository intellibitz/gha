...
impl GemiEngine {
    pub fn generate_reasoning(prompt: &str, workspace: &Path) -> String {
        ...
        if let Some(best_model) = models.get(0) {
             // Example: Pick Ollama for localized data missions if available
        ...
        if let Ok(key) = std::env::var("GEMINI_API_KEY") {
            ...
            let out = Command::new("curl")
                .args(["-s", &url, "-H", "Content-Type: application/json", "-d", &payload.to_string()])
                .output();
    ...
    fn native_synthesis(prompt: &str) -> String {
        ...
        format!("🧠 [Native Synthesis ({} CPUs | {})]:\nMission: \"{}\"\nNote: Swarm is scouting for specialized brains. Native protocol engaged.", cpus, gpu, prompt)
    }
