...
impl GemiEngine {
    ...
    fn reason_internal(prompt: &str, workspace: &Path, allow_reflex: bool) -> String {
        ...
        "❌ ERROR: All intelligence providers (Cloud & Local) are currently unavailable or rate-limited.".to_string()
    }

    fn scout_cloud_providers(prompt: &str) -> Option<String> {
        let mut errors = Vec::new();

        // Priority 1: Groq (Allam model is often free/available)
    ...
    fn execute_groq(prompt: &str) -> Result<String> {
        ...
        Ok(format!("☁️ [🏆 Premier Pick: Groq Allam]:\n{}", Self::cleanse_artifact(text)))
    }
