...
impl ToolRegistry {
    ...
    fn bootstrap(&self) {
        ...
        Self::register_meta_tool(&mut tools, "status", "GHA Substrate status report", MetaCategory::SystemPrimitive, |_arg, _ws| {
            let hardware = HardwareProfiler::get_profile();
            let mut out = format!("GHA Engine Version: {}\n", crate::GHA_VERSION);
        ...
        Self::register_meta_tool(&mut tools, "identity", "GHA Alpha Brain identity report", MetaCategory::SystemPrimitive, |_arg, workspace| {
        ...
        Self::register_meta_tool(&mut tools, "version", "Get engine version", MetaCategory::SystemPrimitive, |_arg, _ws| {
            Ok(format!("gha v{}", crate::GHA_VERSION))
        });
        ...
        Self::register_meta_tool(&mut tools, "web_search_download", "Fetch text content from target URL", MetaCategory::WorkspaceIo, |arg, workspace| {
            ...
            if let Ok(resp) = ureq::get(clean_url).set("User-Agent", "GHA-Substrate/0.1").timeout(std::time::Duration::from_secs(15)).call() {
    ...
    fn register_meta_tool<F>(
        tools: &mut HashMap<String, Arc<dyn GhaTool>>,
        name: &str,
        desc: &str,
    ...
    pub fn execute_tool(name: &str, arg: &str, workspace: &Path) -> String {
        ...
        if let Some(tool) = tools.get(name) {
            ...
            format!("[FAIL] Meta Tool '{}' not found in GHA Registry.", name)
        }
    }
}
