...
12. **Source of Truth**: Core engine behavior and global rules are defined in `AGENTS.md` and `PROJECTS.md`. `gha` also respects target workspace configurations (such as `.gitignore` and build files) dynamically.
...
14. **Native Intent Evolutionary Protocol**: When provided with a user request or intent, the agent delegates execution to the native `gha` binary within the target workspace. If `gha` encounters a failure or capability gap, the agent intervenes, diagnoses the root cause, and triggers the **Motion Rule** to natively harden and add the required functionality directly to the `gha` source code.
15. **Creator Aspiration Rule**: When provided with a creator directive or architectural goal, the agent focuses on future `gha` capabilities and long-term vision. Both user intents and creator aspirations trigger the **Motion Rule**, advancing `gha` evolution one step forward upon successful compilation, audit, and deployment.
