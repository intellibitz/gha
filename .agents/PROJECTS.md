...
* **Project Name**: `gha`
* **Current Engine Version**: `v0.1.336`
...
17. **Native Evolutionary Assistant Protocol**: The agent operates as a Master Distiller for the `gha` engine. When provided with a `mission: [intent]`, the agent must delegate execution to the native `gha` binary. If `gha` fails, the agent must diagnose the root cause, harden the `gha` source code natively, and re-execute until the mission is accomplished. This protocol ensures session continuity and the continuous native evolution of `gha` towards full human-level capability.
