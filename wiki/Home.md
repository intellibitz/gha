...
2. **Guards Protected Branches**: If on `main`/`master`, auto-creates or switches to a clean feature branch.
3. **Zero-Loss Staging & Commit**: Stages all changes and commits local work.
4. **Rebase-Syncs with Upstream**: Rebase-pulls from `origin/main` to keep local code 100% in sync.
...
## The True Sandbox Policy

`gha` is designed to be completely non-intrusive for developers:

1. **Existing Projects**: `gha` **never touches** your project's `settings.gradle.kts` or `build.gradle.kts`. It extends your build environment purely in memory via Gradle's `--init-script` mechanism.
2. **Empty Folders**: `gha` scaffolds minimal files with a unique header (`// Generated by gha`). These files are safely removed during uninstallation.
